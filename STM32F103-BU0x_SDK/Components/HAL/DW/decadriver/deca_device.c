...
volatile uint32_t dwt_tx_fail_count = 0;
...
#ifndef JOHHN
uint32_t AIT_dwt_getxtaltrim(void)
{
	return  (_dwt_otpread(XTRIM_ADDRESS)&0x7f);
}
uint32_t AIT_dwt_otpread(uint16_t address)
{
	return  _dwt_otpread(address);
}

int AIT_dwt_otpwriteandverify(uint16_t address, uint32_t *array, uint8_t length)
{
    int i;
		int ret=-1;
    for(i=0; i<length; i++)
    {
			ret=dwt_otpwriteandverify(array[i],address + i);
 ...
 * @param txDataBytes    - Pointer to the user?? buffer containing the data to send.
 * @param txBufferOffset - This specifies an offset in the DW IC?? TX Buffer at which to start writing data.
 *
 * output parameters
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
...
* no return value
* 
* @brief  ??????PDOA?????????Ipatov??STS POA??????????(??PDOA??1??)
* ??????STS poa (PDOA??3)?????PDOA??????????????(POA -????????)
*
 ...
 * @param timeOff - duration of receiver OFF phase, expressed in multiples of 128/125 ?? (~1 ??). Max value is 255.
 *
 * output parameters
 *
 * no return value
...
float dwt_convertrawtemperature(uint8_t raw_temp)
{
    ...
    // the User Manual formula is: Temperature (??) = ( (SAR_LTEMP ?OTP_READ(Vtemp @ 20??) ) x 1.05)        // Vtemp @ 20??
